...
function Game(props) {
  ...
  var _Object$values$reduce = Object.values(score).reduce(function (acc, score) {
    var _acc = Object(_babel_runtime_helpers_esm_slicedToArray__WEBPACK_IMPORTED_MODULE_0__["default"])(acc, 2),
        currentHightest = _acc[0],
        currentLowest = _acc[1];

    debugger;

    if (score.total > currentHightest) {
      return [score.total, currentLowest];
    } else if (score.total < currentLowest) {
      return [currentHightest, score.total];
    } else {
      return [currentHightest, currentLowest];
    }
  }, [0, Infinity]),
  ...
  console.log("high", highestScore); // const creator = players.find(player => player.id === creator_id);
  // let hasWinner = false;
  // let winnerName;
...
//# sourceMappingURL=[id].js.d14f0ff869ead8bb9141.hot-update.js.map