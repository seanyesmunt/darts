...
function updateScore(gameID, userID, number, newScore) {
  return new Promise(function (resolve, reject) {
    db.ref("games/" + gameID).once("value").then(function (snapshot) {
      ...
      newGame.players = newGame.players.length > 2 ? handleThreePlayerGame(newGame.players) : handleTwoPlayerGame(newGame.players);
...
function handleTwoPlayerGame(originalPlayers) {
  var newPlayers = originalPlayers.slice();
  newPlayers = newGame.players.map(function (player) {
    if (player.id !== userID) {
      return player;
    }
...
function handleThreePlayerGame(originalPlayers) {
  var newPlayers = originalPlayers.slice();
...
//# sourceMappingURL=index.js.ddd5483fccbc1cb34417.hot-update.js.map